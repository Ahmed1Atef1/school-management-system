...
if ($search !== '') {
    $countStmt = $conn->prepare("SELECT COUNT(*) as total FROM class_rooms WHERE name LIKE ? OR location LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT id, name, location, capacity FROM class_rooms WHERE name LIKE ? OR location LIKE ? ORDER BY name LIMIT ? OFFSET ?");
    $stmt->bind_param("ssii", $searchWildcard, $searchWildcard, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $totalRows = $conn->query("SELECT COUNT(*) as total FROM class_rooms")->fetch_assoc()['total'];

    $stmt = $conn->prepare("SELECT id, name, location, capacity FROM class_rooms ORDER BY name LIMIT ? OFFSET ?");
...
<!-- Header Section -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-1">Classrooms</h2>
            <p class="text-muted small mb-0">Manage classrooms, layouts, physical locations, and seating capacities.</p>
        </div>
        <a href="<?= app_url('modules/admin/classrooms/create.php'); ?>"
            ...
            <table class="table table-hover mb-0 align-middle">
                <thead class="table-light" style="background-color: var(--app-surface-soft) !important;">
                    <tr>
                        ...
                        <th class="py-3" style="color: var(--app-muted); font-size: 0.85rem; font-weight: 700; border-bottom: 1px solid var(--app-border);">Name / Room Number</th>
                        <th class="py-3" style="color: var(--app-muted); font-size: 0.85rem; font-weight: 700; border-bottom: 1px solid var(--app-border);">Location</th>
                        <th class="py-3" style="color: var(--app-muted); font-size: 0.85rem; font-weight: 700; border-bottom: 1px solid var(--app-border);">Capacity</th>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td style="border-bottom: 1px solid var(--app-border);">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar-circle d-flex align-items-center justify-content-center fw-bold bg-warning-subtle text-warning rounded-circle" style="width: 32px; height: 32px; font-size: 0.85rem;">
                                        <?= strtoupper(substr($row['name'], 0, 1)); ?>
                                    </div>
                                    <span class="fw-semibold"><?= htmlspecialchars($row['name']); ?></span>
                                </div>
                            </td>
                            <td class="text-muted" style="border-bottom: 1px solid var(--app-border);"><i class="bi bi-geo-alt-fill me-1 text-muted opacity-70"></i> <?= htmlspecialchars($row['location']); ?></td>
                            <td style="border-bottom: 1px solid var(--app-border);">
                                <span class="badge rounded-pill px-3 py-2 fw-semibold" style="background: var(--app-surface-soft); color: var(--app-text); border: 1px solid var(--app-border) !important;">
                                    <i class="bi bi-people-fill me-1 text-muted"></i> <?= htmlspecialchars($row['capacity']); ?> Students
                                </span>
                            </td>
                            <td class="pe-4 text-end" style="border-bottom: 1px solid var(--app-border);">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="<?= app_url('modules/admin/classrooms/update.php?id=' . $row['id']); ?>"
                                       class="btn btn-warning btn-sm rounded-pill d-inline-flex align-items-center gap-1">
                                        <i class="bi bi-pencil-fill"></i> Edit
                                    </a>

                                    <a href="<?= app_url('modules/admin/classrooms/delete.php?id=' . $row['id']); ?>"
    ...
    <?php else: ?>
        <div class="empty-state card border-0 shadow-sm p-5 text-center mb-4" style="background: var(--app-surface); border: 1px solid var(--app-border) !important; border-radius: 28px;">
            <div class="empty-state-icon mb-3">
                <i class="bi bi-door-open-fill text-muted" style="font-size: 3rem; opacity: 0.6;"></i>
            </div>
            <?php if ($search !== ''): ?>
            ...
            <?php else: ?>
                <h4 class="empty-state-title fw-bold">No Classrooms Found</h4>
                <p class="empty-state-text text-muted">Get started by adding your first classroom to the system.</p>
                <div class="d-flex justify-content-center">
                    <a href="<?= app_url('modules/admin/classrooms/create.php'); ?>" class="btn btn-success rounded-pill">
                        <i class="bi bi-plus-lg"></i> Add Classroom
                    </a>
                </div>
            <?php endif; ?>
