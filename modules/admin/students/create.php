...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (empty($baseUsername)) $baseUsername = 'student';
        ...
        if ($stmtCheckEmail->get_result()->num_rows > 0) {
            ...
            try {
                ...
                $stmtUser->execute();
                $userId = $conn->insert_id; // Get the user ID for enrollments

                // 2. Create Student Profile
                $stmtStudent = $conn->prepare("INSERT INTO students (name, email, phone) VALUES (?, ?, ?)");
                $stmtStudent->bind_param("sss", $name, $email, $phone);
                $stmtStudent->execute();
        ...
        <div class="col-md-8 col-lg-7">
            <div class="card shadow-sm border-0" style="background: var(--app-surface); border: 1px solid var(--app-border) !important; border-radius: 18px;">
                <div class="card-body p-4 p-md-5">
                    <h3 class="mb-4 fw-bold">Add Student</h3>
                    ...
                    <?php if ($generatedCredentials): ?>
                        <div class="alert alert-success rounded-3 mb-4">
                            <h5 class="alert-heading fw-bold"><i class="bi bi-check-circle-fill me-2"></i>Student Created!</h5>
                            ...
                            <p class="mt-2 mb-0 small"><em>Please copy these credentials and provide them to the student. The password cannot be recovered once you leave this page.</em></p>
                        </div>
                    <?php endif; ?>
                    ...
                    <form method="post">
                        <div class="mb-4">
                            ...
                            <input name="name" class="form-control form-control-lg fs-6" placeholder="John Doe" value="<?= htmlspecialchars($name ?? ''); ?>" required>
                        </div>

                        <div class="row mb-4">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Email Address</label>
                                <input name="email" type="email" class="form-control" placeholder="john@example.com" value="<?= htmlspecialchars($email); ?>" required>
                        ...
                        <div class="mb-5">
                            <label class="form-label fw-semibold mb-3">Enroll in Courses</label>
                            <div class="row g-3">
                        ...
                        <div class="d-flex gap-2 pt-2 border-top" style="border-color: var(--app-border) !important;">
                            <button type="submit" class="btn btn-primary px-4 py-2 mt-3 rounded-pill fw-medium">Save Student</button>
