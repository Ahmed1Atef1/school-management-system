...
if ($search !== '') {
    $countStmt = $conn->prepare("SELECT COUNT(*) as total FROM students WHERE name LIKE ? OR email LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT id, name, email, phone FROM students WHERE name LIKE ? OR email LIKE ? LIMIT ? OFFSET ?");
    $stmt->bind_param("ssii", $searchWildcard, $searchWildcard, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $totalRows = $conn->query("SELECT COUNT(*) as total FROM students")->fetch_assoc()['total'];

    $stmt = $conn->prepare("SELECT id, name, email, phone FROM students LIMIT ? OFFSET ?");
...
<!-- Header Section -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">Manage and track student details across the school system.</p>
        </div>
        <a href="<?= app_url('modules/admin/students/create.php'); ?>"
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td style="border-bottom: 1px solid var(--app-border);">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar-circle d-flex align-items-center justify-content-center fw-bold bg-primary-subtle text-primary rounded-circle" style="width: 32px; height: 32px; font-size: 0.85rem;">
                            ...
                            <td class="pe-4 text-end" style="border-bottom: 1px solid var(--app-border);">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="<?= app_url('modules/admin/students/update.php?id=' . $row['id']); ?>"
                                       class="btn btn-warning btn-sm rounded-pill d-inline-flex align-items-center gap-1">
                                        <i class="bi bi-pencil-fill"></i> Edit
                                    </a>

                                    <a href="<?= app_url('modules/admin/students/delete.php?id=' . $row['id']); ?>"
    ...
    <?php else: ?>
        <div class="empty-state card border-0 shadow-sm p-5 text-center mb-4" style="background: var(--app-surface); border: 1px solid var(--app-border) !important; border-radius: 28px;">
            <div class="empty-state-icon mb-3">
                <i class="bi bi-people-fill text-muted" style="font-size: 3rem; opacity: 0.6;"></i>
            </div>
            <?php if ($search !== ''): ?>
            ...
            <?php else: ?>
                <h4 class="empty-state-title fw-bold">No Students Found</h4>
                <p class="empty-state-text text-muted">Get started by adding your first student to the system.</p>
